--- src/event-loop.c.orig	2013-07-02 07:08:08.000000000 +0200
+++ src/event-loop.c	2013-08-20 13:18:05.000000000 +0200
@@ -20,6 +20,8 @@
  * OF THIS SOFTWARE.
  */
...
@@ -66,6 +87,7 @@ struct wl_event_source_fd {
 	int fd;
 };
...
+#ifdef HAVE_SYS_EPOLL_H
 ...
 wl_event_source_fd_dispatch(struct wl_event_source *source,
 			    struct epoll_event *ep)
@@ -85,11 +107,32 @@ wl_event_source_fd_dispatch(struct wl_ev
...
@@ -113,7 +156,7 @@ add_source(struct wl_event_loop *loop,
 		ep.events |= EPOLLOUT;
 	ep.data.ptr = source;
...
+	if (epoll_ctl(loop->event_fd, EPOLL_CTL_ADD, source->fd, &ep) < 0) {
 		...
 		free(source);
 		return NULL;
@@ -121,6 +164,47 @@ add_source(struct wl_event_loop *loop,
...
@@ -135,13 +219,14 @@ wl_event_loop_add_fd(struct wl_event_loo
 		return NULL;
...
+#ifdef HAVE_SYS_EPOLL_H
 ...
 {
@@ -155,27 +240,63 @@ wl_event_source_fd_update(struct wl_even
 		ep.events |= EPOLLOUT;
 	ep.data.ptr = source;
...
@@ -196,15 +317,28 @@ wl_event_loop_add_timer(struct wl_event_
 		return NULL;
...
@@ -225,12 +373,20 @@ struct wl_event_source_signal {
...
@@ -238,6 +394,7 @@ wl_event_source_signal_dispatch(struct w
...
@@ -255,6 +412,9 @@ wl_event_loop_add_signal(struct wl_event
 {
 	struct wl_event_source_signal *source;
 	sigset_t mask;
...
+#endif
 	...
 	if (source == NULL)
@@ -262,15 +422,37 @@ wl_event_loop_add_signal(struct wl_event
...
@@ -311,6 +493,7 @@ wl_event_source_check(struct wl_event_so
...
@@ -329,7 +512,83 @@ wl_event_source_remove(struct wl_event_s
...
@@ -429,11 +715,47 @@ wl_event_loop_dispatch(struct wl_event_l
